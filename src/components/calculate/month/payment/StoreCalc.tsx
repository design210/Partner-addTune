...
const StoreCalc = (): JSX.Element => {
  const { t } = useTranslation()
  const { storeMonthSettlement } = useRecoilValue(monthsDetailsState)
  ...
  return (
    <>
      {storeMonthSettlement.list.length > 0 ? (
        <section className={`${styles.containerTbl} ${styles.mt60}`}>
          ...
          <h3 className={styles.mt10}>
            ...
            {t('monthCalcAmount')}&nbsp;
            <span>
              {storeMonthSettlement.total.toLocaleString()}
              {t('won')}
            </span>
          </h3>
          <table className={`${styles.calcDetailTbl} ${styles.mt30}`} ref={tblWrapper}>
            ...
            {storeMonthSettlement.list.map((ele, index) => (
              <tbody key={index}>
                <tr>
                  ...
                  <td className={`${styles.right} ${styles.pr10}`}>{ele.rentFee.toLocaleString()}</td>
          ...
          <p className={`${styles.vat} ${styles.mt5}`}>({t('commission')} 10%)</p>
        </section>
      ) : (
        <></>
      )}
