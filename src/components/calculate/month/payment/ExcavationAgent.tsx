...
const ExcavationAgent = (): JSX.Element => {
  const { t } = useTranslation()
  const { storeAgentMonthSettlement } = useRecoilValue(monthsDetailsState)
  ...
  return (
    <>
      {storeAgentMonthSettlement.list.length > 0 ? (
        <section className={`${styles.containerTbl} ${styles.mt60}`}>
          <h2>{t('storeExcavation')}</h2>
          <h3 className={styles.mt10}>
            {months}
            {t('monthCalcAmount')}&nbsp;
            <span>
              {storeAgentMonthSettlement.total.toLocaleString()}
              {t('won')}
            </span>
          </h3>
          <table className={`${styles.calcDetailTbl} ${styles.mt30}`} ref={tblWrapper}>
            ...
            {storeAgentMonthSettlement.list.map((ele, index) => (
              <tbody key={index}>
                <tr>
...
export default ExcavationAgent
